...
import { OrderDetail, formatDate } from "./order-types"
...
export function OrderCustomerInfo({ customer, user }: OrderCustomerInfoProps) {
  return (
    <div className="bg-card/50 rounded-xl border border-border overflow-hidden">
      ...
      <div className="p-6">
        ...
        <div className="space-y-3">
          ...
          {customer.nickname && customer.name && !customer.name.includes(customer.nickname) && (
            <div className="flex items-center gap-3">
              <UserCircle className="w-5 h-5 text-muted-foreground" />
              <span className="text-foreground">ชื่อเล่น: {customer.nickname}</span>
            </div>
          )}
