...
function ProductOrderItem({
  ...
  return (
    <div className="flex items-center gap-4 p-3 rounded-lg bg-card/50 border border-border">
      {/* Rank */}
      <div
        className={`flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full ${isTop3 ? MEDAL_BG[index] : "bg-secondary"}`}
      >
        <span
          className={`text-sm font-bold ${isTop3 ? MEDAL_COLORS[index] : "text-muted-foreground"}`}
        >
          {index + 1}
        </span>
      </div>
      ...
      <div className="flex-shrink-0 text-right">
        <p className="font-bold text-lg text-primary">
          ...
          <span className="text-sm font-normal">ใบจอง</span>
        </p>
...
function ProductViewItem({
  product,
  index,
  max,
}: {
  product: TopProductByView
  index: number
  max: number
}) {
  const isTop3 = index < 3
