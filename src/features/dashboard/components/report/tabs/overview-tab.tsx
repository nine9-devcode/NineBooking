...
export function OverviewTab({ reportData }: OverviewTabProps) {
  return (
    <div className="space-y-6">
      ...
      <div className="bg-background border border-border rounded-xl p-4">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">
          สถิติช่วงเวลาที่เลือก
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-xl">
          <StatCard
            icon={ShoppingCart}
            color="pink"
            label="จองในช่วงที่เลือก"
            value={reportData.currentPeriodOrders}
            unit="รายการ"
            trend={reportData.orderTrend}
          />
          <StatCard
            icon={Eye}
            color="teal"
            label="ชมในช่วงที่เลือก"
            value={reportData.currentPeriodViews}
            unit="ครั้ง"
            trend={reportData.viewTrend}
          />
        </div>
      </div>
