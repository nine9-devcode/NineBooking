...
export function YearlyChart() {
  ...
  return (
    <div className="bg-background rounded-xl p-6 border border-border">
      ...
      {/* Summary Card */}
      <div
        className={`rounded-lg p-4 mb-6 ${activeTab === "orders" ? "bg-primary/10" : "bg-info/10"}`}
      >
        <div className="flex items-center justify-between">
          <div>
            <p className="text-muted-foreground text-sm mb-1">
              {currentConfig.label}ทั้งปี {selectedYear}
            </p>
            <p
              className={`text-3xl font-bold ${activeTab === "orders" ? "text-primary" : "text-info"}`}
            >
              {currentConfig.total.toLocaleString()}
            </p>
          </div>
          <currentConfig.icon
            className={`w-12 h-12 ${activeTab === "orders" ? "text-primary/30" : "text-info/30"}`}
          />
        </div>
      </div>
