...
export async function GET(request: NextRequest) {
  try {
    ...
    const toBEDate = (d: Date) =>
      `${format(d, "d MMMM", { locale: th })} ${d.getFullYear() + 543}`
    ...
    const periodData = [
      ...
      [
        "แนวโน้มคำสั่งจอง",
        `${data.orderTrend > 0 ? "+" : ""}${data.orderTrend}%`,
        data.dateRange.comparisonLabel,
      ],
      [
        "แนวโน้มยอดเข้าชม",
        `${data.viewTrend > 0 ? "+" : ""}${data.viewTrend}%`,
        data.dateRange.comparisonLabel,
      ],
    ]
    ...
    const catHeaders = sheet3.addRow([
      "อันดับ",
      "หมวดหมู่",
      "จำนวนจอง",
      "% จากคำสั่งจอง",
    ...
    if (data.contactIssueStats.issuesByCategory.length > 0) {
      ...
      data.contactIssueStats.issuesByCategory.forEach((item, i) => {
        const pct =
          data.contactIssueStats.issuesInPeriod > 0
    ...
    const memberData = [
      ...
      [
        "สมาชิกใหม่ช่วงก่อนหน้า",
        data.memberGrowthStats.newMembersComparePeriod,
        data.dateRange.comparisonLabel,
      ],
    ]
    ...
    if (data.memberGrowthStats.membersByType.length > 0) {
      ...
      data.memberGrowthStats.membersByType.forEach((item, i) => {
        const pct =
          totalMembers > 0 ? `${((item.count / totalMembers) * 100).toFixed(1)}%` : "0%"
